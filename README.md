...
별점이 후한 쪽은 오히려 **저가·대량 리뷰 상품**이고, 별점이 가장 박한 중가군이 텍스트 감정은 가장 좋아
괴리가 가장 작습니다. 전체의 75.8%가 별점 4.0 이상이라 별점만으로는 변별이 어렵습니다.
...
├── segment_naming.py                   # 세그먼트 이름을 군집 통계에서 유도
...
K-Means의 클러스터 번호는 초기화에 따라 임의로 붙습니다. 따라서 이름을 번호에 고정하지 않고
[`segment_naming.py`](segment_naming.py)에서 **군집 통계로 유도**합니다.

1. 평균 정가가 가장 높은 군집 → 프리미엄 베스트셀러
2. 나머지 중 괴리가 큰 군집 → 관심 필요 상품 (별점 인플레이션)
3. 나머지 → 가성비 호평 상품

> 이전에는 `{"0": "관심 필요 상품", ...}` 형태로 번호에 이름을 하드코딩해 0번과 2번의 이름이
> 뒤바뀐 채 배포된 적이 있습니다. 재학습해도 이름이 성격을 따라가도록 규칙 기반으로 바꿨습니다.
...
- **Tab 2 · 리뷰 신뢰도 점수** — 리뷰 텍스트를 입력하면 VADER 감성 · 어휘 다양성 · 길이 · 긍부정 키워드를
  조합한 자체 정의 신뢰도 점수(0~100)를 예측합니다.
- **Tab 3 · 불만 고객 조기 경보** — 감정 · 부정 키워드 · 길이로 불만 위험도를 Normal · Risk · Critical
  세 단계로 분류합니다. 별점이 높아도 본문에 불만이 담긴 리뷰를 조기에 골라냅니다.

> Tab 2 · 3은 허위 리뷰나 불만의 정답 라벨이 없는 **자체 정의 지표**이므로 참고용 보조 신호입니다.
...
| 담당 | 파트 |
|---|---|
| sohee | Tab 1 — 상품 세그먼테이션 · 대시보드 |
| hjm00502-eng | Tab 2 — [리뷰 신뢰도 점수](members/member2/) |
| heeseon25 | Tab 3 — [불만 고객 조기 경보](members/member3/) |
